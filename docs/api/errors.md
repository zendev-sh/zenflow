...
| `DropReasonMailboxFull` | `"mailbox-full"` | The bounded mailbox is at the `MaxMailboxSize` cap; the newest message is rejected (oldest-wins fairness) | Raise `WithMaxMailboxSize`, slow the producer, or speed up the consumer. Default is `DefaultMaxMailboxSize` (10000); pass `WithMaxMailboxSize(0)` for unbounded. |
