...
// Debate Soak - smaller two-step debate variant used for stability
// soak runs. Mirrors the structure of debate.yaml but with a reduced
...
func main() {
	wf, err := zenflow.LoadWorkflow("spec/v1/examples/debate-soak.yaml")
	if err != nil {
		log.Fatal("load: ", err)
	}
