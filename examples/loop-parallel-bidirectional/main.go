...
// Loop Parallel Bidirectional - parallel forEach where each iteration
// registers a namespaced delegation in the root router. The
// coordinator addresses individual iterations via
// `forward_to_agent("loop-stages[i].worker")` and receives namespaced
...
func main() {
	wf, err := zenflow.LoadWorkflow("spec/v1/examples/loop-parallel-bidirectional.yaml")
	if err != nil {
		log.Fatal("load: ", err)
	}
